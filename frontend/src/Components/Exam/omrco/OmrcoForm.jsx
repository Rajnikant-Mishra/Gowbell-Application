...
// import ReactDOM from "react-dom";
// import html2canvas from "html2canvas";
// import { UilBell, UilImport } from "@iconscout/react-unicons";
...
//   // Save PDF to server
//   const savePDFToServer = async (pdfBlob, fileName) => {
//     try {
//       const formData = new FormData();
//       formData.append("pdf", pdfBlob, fileName);

//       const response = await axios.post(
//         `${API_BASE_URL}/api/save/pdf`,
//         formData,
//         {
//           headers: {
...
//       if (response.data.success) {
//         console.log("PDF saved to server:", response.data.message);
//         return true;
//       } else {
//         throw new Error(response.data.error || "Failed to save PDF to server");
//       }
//     } catch (error) {
...
//     // Save PDF to server
//     await savePDFToServer(pdfBlob, fileName);
...
//       // Save form data to server before generating PDF
//       const payload = [
//         {
//           country: countryName,
//           state: stateName,
...
//       const validStudents = await generatePDF(response.data.students);

//       // Show success alert
//       Swal.fire({
//         icon: "success",
//         title: "Success!",
//         text: `${validStudents} OMR sheets generated and saved successfully`,
//         confirmButtonColor: "#3085d6",
//         confirmButtonText: "View OMR List",
...
//           error.response?.data?.error ||
//           error.message ||
//           "Failed to generate or save OMR sheets",
//         confirmButtonColor: "#d33",
//         confirmButtonText: "OK",
//       });
//     } finally {
...
//       if (response.data.success) {
//         console.log("PDF saved to server:", response.data.message);
//         return response.data.fileId || fileName;
//       } else {
//         throw new Error(response.data.error || "Failed to save PDF to server");
//       }
//     } catch (error) {
...
//   // Generate PDF and store in localStorage
//   const generatePDF = async (students) => {
//     const doc = new jsPDF({
//       orientation: "portrait",
//       unit: "mm",
...
//       await html2canvas(tempDiv, { scale: 1, quality: 0.5 }).then((canvas) => {
...
//     const fileId = await savePDFToServer(pdfBlob, fileName);
...
//         icon: "error",
//         title: "Storage Error",
//         text: "Storage limit exceeded. Clearing old data and retrying.",
//         confirmButtonColor: "#3085d6",
//         confirmButtonText: "OK",
//       });
//       localStorage.removeItem("downloadHistory");
//       try {
//         localStorage.setItem("downloadHistory", JSON.stringify([newDownload]));
//         setDownloadHistory([newDownload]);
//         window.dispatchEvent(new Event("storage"));
//       } catch (retryError) {
//         throw new Error(
//           "Failed to save download history after clearing storage"
//         );
//       }
//     }
...
//     return { validStudents, fileName, pdfDataUrl };
//   };
...
//       const { validStudents, fileName, pdfDataUrl } = await generatePDF(
...
//         icon: "success",
//         title: "Success!",
//         text: `${validStudents} OMR sheets generated and saved to local storage.`,
//         confirmButtonColor: "#3085d6",
//         confirmButtonText: "View OMR List",
//         timer: 4000,
...
//         icon: "error",
//         title: "Error",
//         text: error.message || "Failed to generate or save OMR sheets",
//         confirmButtonColor: "#d33",
//         confirmButtonText: "OK",
//       });
//     } finally {
...
//   // Generate PDF, store in localStorage, and trigger download
//   const generatePDF = async (students) => {
//     const doc = new jsPDF({
//       orientation: "portrait",
//       unit: "mm",
...
//     // Set progress to 100% and trigger download
//     localStorage.setItem("pdfProgress", JSON.stringify({ progress: 100 }));
//     window.dispatchEvent(new Event("storage"));
...
//         icon: "success",
//         title: "Success!",
//         text: `${validStudents} OMR sheets generated, saved, and downloaded.`,
//         confirmButtonColor: "#3085d6",
//         confirmButtonText: "View OMR List",
//         timer: 4000,
//         timerProgressBar: true,
...
//       await html2canvas(tempDiv, { scale: 2 }).then((canvas) => {
//         const imgData = canvas.toDataURL("image/jpeg", 0.1); // Improved quality
//         const imgWidth = doc.internal.pageSize.getWidth() - 20;
//         const imgHeight = (canvas.height * imgWidth) / canvas.width;
//         doc.addImage(imgData, "JPEG", 10, 10, imgWidth, imgHeight);
//         document.body.removeChild(tempDiv);
...
//     return { validStudents, filename, pdfBlob };
//   };
...
//         icon: "success",
//         title: "Success!",
//         text: `${validStudents} OMR sheets generated, saved, and downloaded. Filename: ${saveResponse.data.filename}`,
//         confirmButtonColor: "#3085d6",
//         confirmButtonText: "View OMR List",
//         timer: 4000,
//         timerProgressBar: true,
...
const handleSave = async () => {
  ...
  try {
    ...
    const countryName = countries.find((c) => c.id === selectedCountry)?.name;
    const stateName = filteredStates.find((s) => s.id === selectedState)?.name;
    const districtName = filteredDistricts.find(
      (d) => d.id === selectedDistrict
    )?.name;
    const cityName = filteredCities.find((c) => c.id === selectedCity)?.name;
    ...
    try {
      ...
      const updateResponse = await axios.put(
        `${API_BASE_URL}/api/omr/omr/filename/${recordId}`, // Corrected endpoint
        updateFormData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
      ...
      if (updateResponse.status !== 200 || !updateResponse.data.message) {
        throw new Error(
          updateResponse.data.error || "Failed to update OMR data with PDF on server"
        );
      }

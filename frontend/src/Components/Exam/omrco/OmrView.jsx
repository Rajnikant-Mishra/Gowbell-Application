...
//               (c) =>
//                 c.id === classValue ||
//                 c.id.toString() === classValue.toString(),
//             );
...
const OmrView = () => {
  ...
  // ==================== getStudentData (your original logic) ====================
  const getStudentData = (studentIds, classes, subjects, level) => {
    if (!studentIds || !subjects) return [];
    ...
    ids.forEach((id) => {
      ...
      if (classData.length > 0 && classData.every((c) => c.id && c.name)) {
        const matchedClass = classData.find(
          (c) =>
            c.id === student.class_id ||
            c.id.toString() === student.class_id?.toString(),
        ...
        if (matchedClass) {
          ...
          classValue = student.class_name;
        }
      } else if (parsedClasses.length > 0) {
        className = student.class_name || parsedClasses[0]?.name || "Unknown";
        classValue = student.class_id || parsedClasses[0]?.id || "Unknown";
      } else {
        className = student.class_name || "Unknown";
        classValue = className;
      }
      ...
      const subjectNames = student.subject_names
        ? student.subject_names.split(",").map((n) => n.trim())
        : parsedSubjects;

      const subjectIds = student.subject_ids
        ? student.subject_ids.split(",").map((i) => i.trim())
        : parsedSubjects.map((s) => (typeof s === "object" && s.id ? s.id : s));
  ...
  }, [omrData, students, classData, subjectData]);
  ...
  // ==================== Download Handler (your original logic) ====================
  const handleDownload = async (studentId, classValue, rowData) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/get/student/${studentId}`,
      ...
      pdf.addImage(imgData, "JPEG", 0, 0, imgWidth, imgHeight);
      pdf.save(`OMR_${studentId}_${subjectName.replace(/\s+/g, "_")}.pdf`);

      root.unmount();
      document.body.removeChild(omrContainer);
    } catch (err) {
      setError("Failed to generate OMR sheet");
      console.error(err);
    }
  };
    ...
    return (
      <Mainlayout>
        <Box sx={{ textAlign: "center", mt: 10 }}>
          <CircularProgress />
          <Typography mt={2}>Loading OMR details...</Typography>
        </Box>
      </Mainlayout>
    );
  }
      ...
      <Box p={3}>
        <Paper sx={{ p: 3, borderRadius: 3, boxShadow: 3 }}>
          ...
          <Typography variant="body1" sx={{ mb: 3 }}>
            <strong>School:</strong> {omrData.school || "N/A"}
          </Typography>
          ...
          {studentData.length > 0 ? (
            <>
              ...
              >
                <Table sx={{ minWidth: 900, border: "none" }}>
                  <TableHead>
                    <TableRow
                      sx={{
                        background: "linear-gradient(90deg, rgb(17 61 236))",
                      }}
                    >
                      <Checkbox
                        sx={{ color: "white" }}
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <Button
                            ...
                            size="small"
                            onClick={() =>
                              handleDownload(row.id, row.class, row)
                            }
                            sx={{
                              borderColor: "#1230ae",
                              color: "#1230ae",
                              "&:hover": { bgcolor: "#1230ae", color: "white" },
                            }}
                          >
                            Download
                          </Button>
              ...
              >
                ...
                <Typography variant="body2" fontWeight={600}>
                  ...
                  {Math.min(page * pageSize, studentData.length)} of{" "}
                  {studentData.length} entries
                </Typography>
                    ...
                    .map((p, idx, arr) => (
                      <React.Fragment key={p}>
                        ...
                        <Button
                          ...
                          sx={{
                            minWidth: 36,
                            backgroundColor: page === p ? "#1230ae" : undefined,
                          }}
                        >
                          {p}
                        </Button>
            ...
            >
              No student data found for this OMR.
            </Typography>
          )}

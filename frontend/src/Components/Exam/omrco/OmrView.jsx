...
//         ? student.subject_ids.split(",").map((id) => id.trim())
//         : parsedSubjects.map((s) => (typeof s === "object" && s.id ? s.id : s));

//       subjectNames.forEach((subjectName, index) => {
//         const subjectId = subjectIds[index] || "Unknown";
//         // Resolve subject name from parsedSubjects if available
//         const subjectObj = parsedSubjects.find(
//           (s) =>
//             (typeof s === "object" && s.id && s.id === subjectId) ||
//             (s.name && s.name.toLowerCase() === subjectName.toLowerCase()) ||
//             s === subjectName
//         );
//         const resolvedSubjectName = subjectObj
//           ? subjectObj.name || subjectObj
//           : subjectName;

//         const combinationKey = `${id}-${classValue}-${subjectId}`;
//         if (!seenCombinations.has(combinationKey)) {
//           studentData.push({
//             id,
//             name: student.student_name,
...
//             class_name: className,
//             school_name: student.school_name || omrData?.school || "Unknown",
//             subject_ids: student.subject_ids || subjectId,
//             class_id: student.class_id || classValue,
//             date: null, // Explicitly set to null as per requirement
//           });
//           seenCombinations.add(combinationKey);
//         }
//       });
...
//       // Resolve subject name from subject_id using omrData.subjects
//       let subjectName = rowData.subject || "Unknown";
//       if (rowData.subject_id && omrData?.subjects) {
...
//         const subjectObj = parsedSubjects.find(
//           (s) =>
//             (typeof s === "object" && s.id && s.id === rowData.subject_id) ||
//             s === rowData.subject_id
//         );
//         subjectName = subjectObj
//           ? subjectObj.name || subjectObj
//           : rowData.subject ||
//             apiData.subject_names?.split(",")[0] ||
//             "Unknown";
//       }
...
//         subject: subjectName,
//         className: className,
//         date: null, // Set to null as per requirement
//         rollNumber: rowData.roll_no || apiData.roll_no || "N/A",
//         subjectIds:
//           rowData.subject_ids ||
//           apiData.subject_ids ||
...
//       pdf.addImage(imgData, "PNG", 0, 0, imgWidth, imgHeight);
//       pdf.save(`omr_sheet_${studentId}_${subjectName}.pdf`);
...
const OmrView = () => {
  ...
  const [students, setStudents] = useState([]);
  const [classData, setClassData] = useState([]);
  const [subjectData, setSubjectData] = useState([]); // New state for subject data
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [omrResponse, studentsResponse, classResponse, subjectResponse] =
          await Promise.all([
            axios.get(`${API_BASE_URL}/api/omr/get/${id}`),
            axios.get(`${API_BASE_URL}/api/get/allstudents`),
            axios.get(`${API_BASE_URL}/api/class`),
            axios.get(`${API_BASE_URL}/api/subject`), // Fetch subject data
          ]);
        setOmrData(omrResponse.data);
        setStudents(studentsResponse.data);
        setClassData(classResponse.data);
        setSubjectData(subjectResponse.data); // Set subject data
        setLoading(false);
      } catch (err) {
        setError(err.response?.data?.message || "Failed to fetch data");
        setLoading(false);
  ...
  const getStudentData = (studentIds, classes, subjects, level) => {
    ...
    ids.forEach((id) => {
      ...
      subjectNames.forEach((subjectName, index) => {
        const subjectId = subjectIds[index] || "Unknown";
        // Resolve subject name from subjectData
        const subjectObj = subjectData.find(
          (s) =>
            (s.id && (s.id === subjectId || s.id.toString() === subjectId)) ||
            (s.name && s.name.toLowerCase() === subjectName.toLowerCase())
        );
        const resolvedSubjectName = subjectObj
          ? subjectObj.name
  ...
  const handleDownload = async (studentId, classValue, rowData) => {
    try {
      ...
      // Resolve subject name from subjectData using subject_id
      let subjectName = rowData.subject || "Unknown";
      if (rowData.subject_id && subjectData.length > 0) {
        const subjectObj = subjectData.find(
          (s) =>
            s.id &&
            (s.id === rowData.subject_id ||
              s.id.toString() === rowData.subject_id)
        );
        subjectName = subjectObj
          ? subjectObj.name
          : rowData.subject ||
      ...
      const omrProps = {
        ...
        //   rowData.subject_id ||
        //   "Unknown",
           subjectIds: apiData.student_subject || "Unknown",
        classId: rowData.class_id || apiData.class_id || classValue,
      };
  ...
  //       classData.every((c) => typeof c === "object" && c.id && c.name)
  //     ) {
  //       const matchedClass = classData.find((c) => c.id == classValue);
  //       className = matchedClass ? matchedClass.name : classValue;
  //     } else if (omrData?.classes) {
  //       const parsedClasses =
  //         typeof omrData.classes === "string"
  ...
  //         parsedClasses.every((c) => typeof c === "object" && c.id && c.name)
  //       ) {
  //         const matchedClass = parsedClasses.find((c) => c.id == classValue);
  //         className = matchedClass ? matchedClass.name : classValue;
  //       }
  //     }

  //     // Resolve subject name from apiData.student_subject or fallback
  //     let subjectName = apiData.student_subject || rowData.subject || "Unknown";
  //     let subjectColor = "black"; // Default color, update based on API structure
  //     if (rowData.subject_id || apiData.subject_id) {
  //       const subjectId = rowData.subject_id || apiData.subject_id;
  //       if (subjectData?.length > 0) {
  //         const subjectObj = subjectData.find((s) => s.id && s.id == subjectId);
  //         if (subjectObj) {
  //           subjectName = subjectObj.name || subjectName;
  //           subjectColor = subjectObj.color || subjectColor; // Assuming subjectData might include color
  //         }
  //       }
  //     } else if (
  //       apiData.student_subject &&
  //       typeof apiData.student_subject === "object"
  //     ) {
  //       // Handle case where student_subject is an object with name and color
  //       subjectName = apiData.student_subject.name || subjectName;
  //       subjectColor = apiData.student_subject.color || subjectColor;
  //     }

  //     // Determine OMR component based on class
  //     const useOMR50 = ["01", "02", "03"].includes(className);
  //     const OMRComponent = useOMR50 ? OMRSheet50 : OMRSheet60;
  ...
  //       level: rowData.level || apiData.level || omrData?.level || "N/A",
  //       subject: subjectName,
  //       subjectColor, // Pass color to OMR component if needed
  //       className,
  //       date: null,
  //       rollNumber: rowData.roll_no || apiData.roll_no || "N/A",
  //       subjectName:
  ...
  //       const checkRender = () => {
  //         if (
  //           omrContainer.querySelector("canvas") ||
  //           omrContainer.children.length > 0
  //         ) {
  //           resolve();
  //         } else {
  ...
  //     document.body.removeChild(omrContainer);
  //   } catch (err) {
  //     console.error("Error generating OMR sheet:", err);
  //     setError(
  //       err.response?.data?.message ||
  //         "Failed to generate OMR sheet. Please try again."
  //     );
  //   }
  // };

...
// import MedalsWinnersList from "../../Exam/Prize-distribution/MedalwinnerList";
...
//     // Calculate percentages for cutoff
//     const subjectName =
...
//     // Calculate cutoff percentages per class
//     const classCutoff = [
//       {
//         class:
//           classes.find((cls) => cls.value === selectedClassId)?.label ||
//           selectedClassId,
...
//       }_Class${selectedClassId}_Subject${subjectName}.pdf`
//     );
//   };
...
const ExaminationForm = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Prepare winners list, filtering only students with medals
    const winnersList = students
      .filter((student) => student.medals && student.medals !== "N/A")
      .map((student, index) => ({
        slNo: index + 1,
        school: student.school_id,
        name: student.student_name || "N/A",
    ...
    if (winnersList.length === 0) {
      toast.error("No students with medals found to download.");
      return;
    }
    ...
    doc.save(
      `Result_${
        schools.find((s) => s.id === selectedSchool)?.school_name || "School"
      }_Class${selectedClassId}_Subject${subjectName}_MedalWinners.pdf`
    );
  };
  ...
  return (
    <Mainlayout>
      <ToastContainer />
      <div className="d-flex justify-content-between align-items-center mb-3">
        <Breadcrumb data={[{ name: "Prize  Distribution", link: "" }]} />
      </div>
      <Container component="main" maxWidth="xl">
        <Paper
          className={`${styles.main}`}
            ...
            >
              ...
              <Box>
                <Button
                  ...
                  disabled={isLoading || students.length === 0}
                >
                  {isLoading ? "Processing..." : "Download Prize List (PDF)"}
                </Button>
              </Box>
            </Box>
            <Table>
              <TableHead>
                <TableRow style={{ backgroundColor: "#c8cdf5ff" }}>
                  <TableCell align="center">STUDENT NAME</TableCell>
                  <TableCell align="center">CLASS</TableCell>
                  <TableCell align="center">SUBJECT</TableCell>
                  <TableCell align="center">ROLL NO</TableCell>

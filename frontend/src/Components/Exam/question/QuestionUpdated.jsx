...
const UpdateQuestionForm = () => {
  ...
  // Fetch question data and initial location data
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [questionRes, countriesRes, statesRes, districtsRes, citiesRes] = 
          await Promise.all([
            axios.get(`${API_BASE_URL}/api/q1/question/${id}`),
        ...
        setFormData({
          school: questionData.school_name || "",
          exam_set: questionData.set_name || "",
        });
        setSelectedCountry(questionData.country_id || "");
        setSelectedState(questionData.state_id || "");
        setSelectedDistrict(questionData.district_id || "");
        setSelectedCity(questionData.city_id || "");
  ...
  useEffect(() => {
    ...
    if (!states.some((s) => s.id === selectedState)) {
      setSelectedState("");
      setSelectedDistrict("");
      setSelectedCity("");
  ...
  useEffect(() => {
    ...
    if (!districts.some((d) => d.id === selectedDistrict)) {
      setSelectedDistrict("");
      setSelectedCity("");
      setFormData({ ...formData, school: "" });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/q1/question/${id}`,
        payload
      );
    ...
    <Mainlayout>
      <Breadcrumb
        data={[
          ...
          { name: "Update Question", link: `/question-update/${id}` },
        ]}
      />

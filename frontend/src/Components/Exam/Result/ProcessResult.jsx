...
//         setClasses(response.data.map((cls) => ({ value: cls.id, label: cls.name })));
//       } catch (error) {
...
//         setSubjects(response.data.map((sub) => ({ value: sub.id, label: sub.name })));
//       } catch (error) {
...
//   // Fetch students
//   const fetchStudents = useCallback(async (updatePending = false) => {
...
//       const response = await axios.post(`${API_BASE_URL}/api/getFilteredStudentreceipt`, {
...
//     await fetchStudents(true); // Call fetchStudents with updatePending=true
//   };
...
//       sortedPercentages[0] !== undefined ? sortedPercentages[0].toFixed(2) + "%" : "N/A";
//     const secondHighestPercentage =
//       sortedPercentages[1] !== undefined ? sortedPercentages[1].toFixed(2) + "%" : "N/A";
//     const thirdHighestPercentage =
//       sortedPercentages[2] !== undefined ? sortedPercentages[2].toFixed(2) + "%" : "N/A";
...
//       gold: highestPercentage,
//       silver: secondHighestPercentage,
//       bronze: thirdHighestPercentage,
//     }));
...
//           <Typography className={`${styles.formTitle} mb-4`}>Process Result</Typography>
//           {fetchError && (
...
//                       <TableCell align="center">{student.student_name || "N/A"}</TableCell>
//                       <TableCell align="center">{student.class_name || "N/A"}</TableCell>
...
//                           : "N/A"}
//                       </TableCell>
//                       <TableCell align="center">{student.roll_no || "N/A"}</TableCell>
//                       <TableCell align="center">{student.full_mark || "N/A"}</TableCell>
//                       <TableCell align="center">{student.mark_secured || "N/A"}</TableCell>
//                       <TableCell align="center">{student.percentage || "N/A"}</TableCell>
//                       <TableCell align="center">{student.ranking || "N/A"}</TableCell>
//                       <TableCell align="center">{student.medals || "N/A"}</TableCell>
//                       <TableCell align="center">{student.remarks || "N/A"}</TableCell>
//                       <TableCell align="center">{student.certificate || "N/A"}</TableCell>
//                       <TableCell align="center">{student.level || "N/A"}</TableCell>
//                       <TableCell align="center" style={getStatusStyle(student.status)}>
//                         {student.status || "N/A"}
//                       </TableCell>
//                     </TableRow>
//                   ))
...
//                   <TableRow>
//                     <TableCell colSpan={12} align="center">
//                       {selectedSchool && selectedClassIds.length && selectedSubjectId
...
//                         {students.length} of {page}-{Math.ceil(totalCount / pageSize)}
//                       </p>
//                     </label>
//                   </div>
...
//                       .filter((pg) => pg === 1 || pg === Math.ceil(totalCount / pageSize) || Math.abs(pg - page) <= 2)
//                       .map((pg, index, array) => (
...
//         // Fetch school details to get school_address
//         const schoolResponse = await axios.get(
//           `${API_BASE_URL}/api/get/schools/${selectedSchool}`
//         );
//         const schoolAddress = schoolResponse.data?.school_address || "N/A";
...
//         const fetchedStudents = studentResponse.data.students || [];
//         const updatedStudents = fetchedStudents.map((student) => ({
//           ...student,
//           student_subject: Array.isArray(student.student_subject)
//             ? student.student_subject
...
//             : ["N/A"],
//           status: student.status || "N/A",
//           school_address: schoolAddress, // Add school_address to each student
//         }));
...
//       }_Classes${selectedClassIds.join("_")}_Subject${selectedSubjectId}.pdf`
//     );
...
//   const [selectedSubjectId, setSelectedSubjectId] = useState("");
//   const [students, setStudents] = useState([]);
//   const [schoolAddress, setSchoolAddress] = useState(""); // Moved schoolAddress to state
//   const [isLoading, setIsLoading] = useState(false);
//   const [page, setPage] = useState(1);
//   const [pageSize, setPageSize] = useState(5);
//   const pageSizes = [5, 10, 25, 50];
...
//         setSchoolAddress(fetchedSchoolAddress); // Store in state
...
//           schools.find((s) => s.id === selectedSchool)?.school_name || "N/A"
//         }
//         schoolAddress={schoolAddress} // Use state variable
//         classId={selectedClassIds.join(",")}
//         subjectId={selectedSubjectId}
//         country={countryName}
//         state={stateName}
...
//     document.body.removeChild(container);
//     root.unmount(); // Clean up React 18 root
//   };
...
//     setSelectedSchool("");
//     setStudents([]);
//     setSchoolAddress(""); // Reset school address
//   }, [selectedCountry, states]);
...
const ExaminationForm = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Get subject name from selectedSubjectId
    const subjectName =
      subjects.find((sub) => sub.value === selectedSubjectId)?.label || "N/A";
    ...
    const component = (
      <MedalsWinnersList
        ...
        classId={selectedClassIds.join(",")}
        subjectId={selectedSubjectId}
        subjectName={subjectName} // Added subjectName prop
        country={countryName}
        state={stateName}
        district={districtName}
        city={cityName}
    ...
    doc.save(
      `Result_${
        schools.find((s) => s.id === selectedSchool)?.school_name || "School"
      }_Classes${selectedClassIds.join("_")}_Subject${subjectName}.pdf` // Updated to use subjectName
    );

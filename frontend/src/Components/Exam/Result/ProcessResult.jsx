...
import styles from "./OmrForm.module.css"; // Adjust path as needed
...
import MedalsWinnersList from "../../Exam/Result/MedalwinnerList"; // Adjust path as needed
...
const OmrForm = () => {
  ...
  const fetchLocationData = async () => {
    try {
      ...
      const india = countriesRes.data.find(
        (country) =>
          country.name?.toLowerCase().trim() === "india"
      );
      if (india) {
  ...
  //   ) {
  //     toast.error(
  //       "Please select school, classes, and subjects to evaluate results"
  //     );
  //     return;
  //   }
  ...
  // Only changes / upgrades are shown. Rest of your code stays the same
  const fetchStudents = useCallback(
    async (updatePending = false) => {
      const session_id = localStorage.getItem("currentSessionId");
      ...
      try {
        ...
        const studentResponse = await axios.post(
          `${API_BASE_URL}/api/getFilteredStudentreceipt`,
          {
            ...
            subjectIds,
            session_id,
            updatePending, // This triggers backend evaluation
          }
        );
        ...
        toast.success(
          updatePending
            ? "Results evaluated & students levels updated successfully"
  ...
  const handleGenerateRank = async () => {
    ...
    await fetchStudents(true); // Trigger evaluation
  };
            ...
            >
              ...
              <Box>
                <Button
                  ...
                  disabled={isLoading}
                >
                  {isLoading ? "Processing..." : "Evaluate Result"}
                </Button>
                <Button
                  variant="contained"
                  sx={{

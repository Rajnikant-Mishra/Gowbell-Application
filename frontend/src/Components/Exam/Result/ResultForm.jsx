...
//     school_id: "", // Changed from school_name to school_id
//     student_name: "",
//     class_id: "",
//     roll_no: "",
//     full_mark: "",
...
//             id: school.id, // Store school ID
//             name: school.name, // Store school name
...
//     setSelectedDistrict("");
//     setSelectedCity("");
//     setFormData({ ...formData, school_id: "" }); // Reset school_id
//     setSchools([]);
...
//     if (name === "school_id") {
//       // When school is selected, store the school_id
...
//     value: school.id, // Use school ID as value
...
//                     name="school_id" // Changed to school_id
...
const ResultForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    level: "",
    subject_id: "",
    session_id: "", // Added session_id to formData
  });
  const [classes, setClasses] = useState([]);
  const [subjects, setSubjects] = useState([]);
  const [countries, setCountries] = useState([]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      Swal.fire({
        ...
        icon: "error",
        title: "Unauthorized",
        text: "Please log in to create a result.",
        showConfirmButton: false,
        timer: 1000,
        timerProgressBar: true,
        toast: true,
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/result/create`,
        payload,
        {
          headers: {

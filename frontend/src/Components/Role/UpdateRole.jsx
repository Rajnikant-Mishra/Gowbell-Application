...
const UpdateRoleForm = () => {
  ...
  useEffect(() => {
    const fetchRole = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/r1/role/${id}`);
        if (response.ok) {
        ...
        Swal.fire({
          title: "Error",
          text: "An unexpected error occurred while fetching role data.",
          icon: "error",
        });
      }
    };
  ...
  return (
    <Mainlayout>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <Breadcrumb
          data={[{ name: "Role", link: "/role-list" }, { name: "Update Role" }]}
        />
      </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <FormControl fullWidth size="small" required>
              ...
              <Select
                ...
                onChange={handleChange}
                renderValue={renderSelectedIcons}
                sx={{ fontSize: "14px" }}
              >

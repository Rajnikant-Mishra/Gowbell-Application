...
export default function FormCo() {
  ...
  const validationSchema = Yup.object({
    ...
    remarks: Yup.string().required("Remarks are required"),
    via: Yup.string().required("Via is required"),
    goodies: Yup.array().min(1, "At least one goodie is required").required(),
  });
  ...
  const formik = useFormik({
    initialValues: {
      ...
      postal_name: "",
      postal_delivery_date: "",
      goodies: [], // will hold objects {inventory_id, item, sub_item, sub_item_name, quantity}
      remarks: "",
    },
    validationSchema,
    onSubmit: async (values) => {
  ...
  useEffect(() => {
    axios
      .get(`${API_BASE_URL}/api/v1/inventory`) // Your inventory API
      .then((res) => {
        // Map inventory for Autocomplete
        const options = res.data.map((item) => ({
          inventory_id: item.id,
          label: `${item.item} - ${item.sub_item} - ${item.sub_item_name}`,
          item: item.item,
    ...
    <Mainlayout>
      ...
      <Box className={`${styles.formContainer} container-fluid pt-5`}>
        <div className={`${styles.formBox}`}>
          ...
          <form onSubmit={formik.handleSubmit} className={styles.formContent}>
            <Grid container spacing={2}>
              {/* Consignment ID, Date, School, Via fields */}
              <Grid item xs={12} sm={6} md={2}>
                <TextInput
                  label="Consignment Id"
                  name="consignment_id"
              ...
              {/* Conditional fields for via (Vehicle, Courier, Postal) */}
              {formik.values.via === "Vehicle" && (
                <>
                  <Grid item xs={12} sm={6} md={4}>
                    <TextInput
              ...
              {/* <Grid item xs={12} sm={6} md={8}>
                <Autocomplete
                  ...
                  getOptionLabel={(option) => option.label}
                  isOptionEqualToValue={(option, value) =>
                    option.inventory_id === value.inventory_id
                  }
                  value={formik.values.goodies}
                  onChange={(e, newValue) =>
                    formik.setFieldValue("goodies", newValue)
                  }
                  renderOption={(props, option, { selected }) => (
                    <li {...props}>
                      <Checkbox
                        checked={formik.values.goodies.some(
                          (g) => g.inventory_id === option.inventory_id
                        )}
                      />{" "}
                      {option.label}
                    </li>
              ...
              <Grid item xs={12} sm={8}>
                <Autocomplete
                  ...
                  value={formik.values.goodies}
                  onChange={(e, newValue) => {
                    // Preserve existing quantities if item was already selected
                    const updatedGoodies = newValue.map((item) => {
                      const existing = formik.values.goodies.find(
                        (g) => g.inventory_id === item.inventory_id
                      );
                      return existing ? existing : { ...item, quantity: "" };
                    });
                    formik.setFieldValue("goodies", updatedGoodies);
                  }}
                  renderOption={(props, option, { selected }) => (
                    <li {...props}>
                      <Checkbox
                ...
                {/* Quantity inputs for each selected goodie */}
                {formik.values.goodies.length > 0 && (
                  <Box mt={2}>
                    <Grid container spacing={2}>
                      {formik.values.goodies.map((goodie, index) => (
                        <React.Fragment key={goodie.inventory_id}>
                          ...
                          <Grid item xs={12} sm={6}>
                            <TextField
                              ...
                              onChange={(e) => {
                                const newGoodies = [...formik.values.goodies];
                                newGoodies[index].quantity = e.target.value;
                                formik.setFieldValue("goodies", newGoodies);
                              }}
                              inputProps={{ min: 1 }}
                            />

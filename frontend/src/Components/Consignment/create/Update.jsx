...
export default function UpdateConsignment() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        await axios.put(`${API_BASE_URL}/api/c1/consignments/${id}`, values, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
        ...
        Swal.fire({
          ...
          background: "#fff",
          customClass: { popup: "small-swal" },
        }).then(() => navigate("/consignment-list"));
      } catch (error) {
        Swal.fire({
          position: "top-end",
          icon: "error",
  ...
  useEffect(() => {
    const token = localStorage.getItem("token");
    axios
      .get(`${API_BASE_URL}/api/c1/consignments/${id}`, {
      ...
      .then((res) => {
        ...
        formik.setValues({
          ...data,
          goodies: Array.isArray(data.goodies)
            ? data.goodies
            : JSON.parse(data.goodies || "[]"),
        });
      })
      .catch((err) => console.error("Error loading consignment:", err));
  }, [id]);
    ...
    <Mainlayout>
      ...
      <Box className={`${styles.formContainer} container-fluid pt-5`}>
        <div className={`${styles.formBox}`}>
          ...
          <form onSubmit={formik.handleSubmit} className={styles.formContent}>
            <Grid container spacing={2}>
              {/* Consignment ID, Date, School, Via */}
              <Grid item xs={12} sm={6} md={2}>
                <TextInput
                  label="Consignment Id"
                  name="consignment_id"

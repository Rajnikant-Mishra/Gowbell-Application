...
function CountryList() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/affiliated")
      .then((response) => {
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .delete(`http://localhost:5000/api/affiliated/${id}`)
          .then((response) => {
            ...
            Swal.fire("Deleted!", "The affiliated  has been deleted.", "success");
          })
          .catch((error) => {
            console.error("Error deleting subject:", error);
            // Show an error alert if deletion fails
            Swal.fire("Error!", "There was an issue deleting the affiliated .", "error");
          });
      }
    });
  };
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="actionButtons d-flex gap-2">
          ...
          <Link to={`/affiliated/update/${row.id}`} type="button" className="">
            <FaRegEdit />
          </Link>
          <button
          type="button"
    ...
    <Mainlayout>
      <div className="container">
        <div className="d-flex justify-content-end align-items-center mb-3">
          ...
          <Link to={"/affiliated/create"} className="btn btn-primary">
            <FaPlus/>
          </Link>
        </div>

...
//           `${API_BASE_URL}/api/center/get-paginate-serach`,
//           {
...
//         const {
//           centers, // Note: backend now returns `centers` instead of `classes`
//           totalRecords,
//           totalPages,
//           currentPage,
//           nextPage,
...
//           throw new Error("Invalid centers data received from API");
//         }

//         const offset = (currentPage - 1) * pageSize;
//         const formattedData = centers.map((record, index) => ({
...
//       } catch (error) {
//         console.error("Error fetching center records:", error);
//         Swal.fire({
//           position: "top-end",
//           icon: "error",
//           title: "Error!",
//           text: "Failed to fetch center data.",
...
//           <Breadcrumb data={[{ name: "Center" }]} />
//         </div>
//         <div>
//           <CreateButton link={"/center-form"} />
//         </div>
//       </div>
//       <div
//         style={{
...
export default function CenterDataTable() {
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [totalRecords, setTotalRecords] = useState(0);
  const [totalPages, setTotalPages] = useState(0);
  const [searchTerm, setSearchTerm] = useState("");
  const [loading, setLoading] = useState(false);
  const [records, setRecords] = useState([]); // ← main data array (centers)
  ...
  // Fetch centers with pagination + search
  useEffect(() => {
    const fetchCenters = async () => {
      ...
      try {
        ...
        const {
          centers = [], // ← important: backend returns "centers"
          totalRecords = 0,
          totalPages = 0,
        } = response.data;

        const formatted = centers.map((center, index) => ({
          ...center,
          sl_no: (page - 1) * pageSize + index + 1,
          created_at: formatTimestamp(center.created_at),
          updated_at: formatTimestamp(center.updated_at || center.updatedAt),
          // Adjust according to your actual backend field names
          name: center.name || center.center_name || "—",
          created_by: center.created_by_username || center.created_by || "—",
        }));
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "This center will be permanently deleted!",
      icon: "warning",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        const token = localStorage.getItem("token");
        axios
          .delete(`${API_BASE_URL}/api/center/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
          .then(() => {
  ...
  return (
    <Mainlayout>
      ...
      >
        <Breadcrumb data={[{ name: "Centers" }]} />
        <CreateButton link="/center/create" /> {/* ← update route if needed */}
      </div>
      ...
      >
        <TextField
          fullWidth
          size="small"
          placeholder="Search by center name, code, location..."
          value={searchTerm}
          onChange={(e) => {
        ...
        {loading ? (
          <div style={{ textAlign: "center", padding: "100px 0" }}>
            ...
            <p style={{ marginTop: 20, color: "#555" }}>Loading centers...</p>
          </div>
        ) : (
          <>
            <TableContainer
                  ...
                  {records.length === 0 ? (
                    ...
                    records.map((row) => (
                      ...
                      >
                        ...
                        <TableCell>{row.center_code}</TableCell>

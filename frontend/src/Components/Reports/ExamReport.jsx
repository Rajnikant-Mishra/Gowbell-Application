...
import styles from "./examReport.module.css"; // Adjust path as needed
import axios from "axios";
...
import "jspdf-autotable";
import html2canvas from "html2canvas";
import MedalsWinnersList from "../Reports/MedalwinnerList"; // Adjust path as needed
import { createRoot } from "react-dom/client";
...
const ExaminationForm = () => {
  ...
  const fetchSchoolsByLocation = async (filters) => {
    ...
    try {
      ...
      console.error("Error fetching schools:", error);
      setSchools([]);
      setFetchError(error.response?.data?.message || "Failed to fetch schools.");
      toast.error(error.response?.data?.message || "Failed to fetch schools.");
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchStudents = useCallback(
    async (updatePending = false) => {
      ...
      if (!session_id) {
        ...
        setStudents([]);
        setTotalCount(0);
        toast.error("No session selected. Please select a session from the header.");
        return;
      }
      ...
      <Container component="main" maxWidth="xl">
        ...
        >
          ...
          <Box mt={4}>
            ...
            <Table>
              <TableHead>
                <TableRow sx={{ backgroundColor: "#1d53ddff" }}>

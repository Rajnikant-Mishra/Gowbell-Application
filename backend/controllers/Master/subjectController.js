...
export const createSubject = (req, res) => {
  const { name, status } = req.body;
  const createdBy = req.user.id; // Get logged-in user ID

  Subject.create(name, status, createdBy, (err, result) => {
    if (err) {
      if (err === "Subject already exists") {
        return res
          .status(400)
    ...
    res.status(201).json({ id: result.insertId, name, status, createdBy });
  });
};
